...
namespace Datos
{
    public class PanelaRepository
    {
        ...
        public Panela BuscarPorNumeroLote(string numeroLote)
        {
            SqlDataReader dataReader;
            using (var command = _connection.CreateCommand())
            {
                command.CommandText = "select * from Panela where NumeroLote=@NumeroLote";
                command.Parameters.AddWithValue("@NumeroLote", numeroLote);
                dataReader = command.ExecuteReader();
                dataReader.Read();
